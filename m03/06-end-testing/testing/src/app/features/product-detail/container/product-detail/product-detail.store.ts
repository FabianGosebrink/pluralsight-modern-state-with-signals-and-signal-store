  ...
  withEffects(
    (
      ...
      loadProduct$: events.on(productDetailPageEvents.opened).pipe(
        ...
        tap(({ payload }) => productDetailApiEvents.productLoadStarted(payload)),
        exhaustMap(({ payload }) =>
          productDetailService.loadProductDetail(payload).pipe(
            mapResponse({
              next: (product) => productDetailApiEvents.productLoadedSuccess(product),
              error: (error: unknown) => productDetailApiEvents.productLoadedFailure(error),
            })
          )
        )
      ),
      ...
      logErrors$: events
        ...
        .pipe(tap(({ payload }) => console.error(`Failed to load product`, payload))),
    })
  )
);
